...
import { Loader2, ArrowLeft, Image as ImageIcon, XCircle, Save } from 'lucide-react';
...
export default function AdminEditProductPage() {
  ...
  useEffect(() => {
    if (!authLoading && isFirebaseConfigured) {
      if (!isAuthenticated) {
        console.log("AdminEditProductPage: Not authenticated, redirecting to /admin/login");
        router.replace('/admin/login');
        setCheckingAuth(false);
      } else {
        const checkAdminStatus = async () => {
          if (!db || !user?.uid) {
            console.error("AdminEditProductPage: Firebase DB or user UID not available for admin check.");
            setIsAdmin(false);
            router.replace('/');
            setCheckingAuth(false);
            return;
          ...
          try {
            ...
            if (snapshot.exists()) {
              ...
              if (user.uid in adminUids && adminUids[user.uid] === true) {
                setIsAdmin(true);
                console.log("AdminEditProductPage: User is authorized admin.");
              } else {
                setIsAdmin(false);
                console.log("AdminEditProductPage: User UID not found in adminUsers or not authorized, redirecting to /");
                router.replace('/');
              }
            } else {
              console.log("AdminEditProductPage: 'adminUsers' node does not exist in Firebase. No admins configured.");
              setIsAdmin(false);
              router.replace('/');
            }
          } catch (error) {
            console.error("AdminEditProductPage: Error checking admin status from Firebase:", error);
            setIsAdmin(false);
            router.replace('/');
          } finally {
            setCheckingAuth(false);
        ...
        console.error("AdminEditProductPage: Firebase is not configured. Admin access disabled.");
        router.replace('/');
        setCheckingAuth(false);
    }
  ...
  useEffect(() => {
    if (isAdmin && isFirebaseConfigured && db && productId) {
  ...
  }, [isAdmin, isFirebaseConfigured, db, productId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!db || !user?.uid || !product) {
      toast({
        title: "Firebase Error",
        description: "Firebase database not available or product not loaded.",
        variant: "destructive",
      });
  ...
  if (fetchingProduct) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-950 text-gray-300">
        <Loader2 className="h-12 w-12 animate-spin text-blue-500" />
        <p className="ml-4 text-xl">Loading Product Details...</p>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-8 pb-4 border-b border-blue-700/30">
          <h1 className="text-4xl font-bold tracking-tight text-blue-400 flex items-center">
            <Save className="h-10 w-10 mr-3 text-yellow-500 animate-pulse" />
            Edit Product: <span className="ml-2 text-white">{product.name}</span>
          </h1>
        ...
        <Card className="bg-gray-900 border border-blue-700/30 shadow-lg shadow-blue-500/20 rounded-xl">
          <CardHeader className="border-b border-blue-700/20 pb-4">
            ...
            <CardDescription className="text-gray-400">Update the information for this product.</CardDescription>
          </CardHeader>
          <CardContent className="p-6">
            <form onSubmit={handleSubmit} className="grid gap-6">
              <div className="grid gap-2">
                <Label htmlFor="productId" className="text-blue-300">Product ID</Label>
                <Input
                  id="productId"
                  value={product.id}
                  readOnly
                  disabled
                  className="bg-gray-800 border border-blue-600/50 text-gray-400 cursor-not-allowed"
                />
              </div>
